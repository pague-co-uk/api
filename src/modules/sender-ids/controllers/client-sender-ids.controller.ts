import {
  Body,
  Controller,
  Delete,
  Get,
  NotFoundException,
  Param,
  ParseUUIDPipe,
  Post,
  Put,
  Query,
} from "@nestjs/common";
import {
  ApiNotFoundResponse,
  ApiOperation,
  ApiParam,
  ApiTags,
} from "@nestjs/swagger";

import { Authorize } from "../../../common/authorization/decorators/authorize.decorator.js";
import { Permissions } from "../../../common/authorization/permissions/permissions.registry.js";
import { PaginatedResponse } from "../../../common/interfaces/paginated.response.js";
import { ApiPaginatedResponse } from "../../../decorators/api-paginated-response.decorator.js";
import { ApiSuccessResponse } from "../../../decorators/api-success-response.decorator.js";

import { CreateSenderIdDto } from "../dto/create-sender-id.dto.js";
import { FindSenderIdsDto } from "../dto/find-sender-ids.dto.js";
import { SenderIdResponseDto } from "../dto/sender-id.response.dto.js";
import { UpdateSenderIdDto } from "../dto/update-sender-id.dto.js";
import { SenderIdMapper } from "../sender-id.mapper.js";
import { SenderIdService } from "../services/sender-id.service.js";

@ApiTags("Sender IDs")
@Controller("clients/:clientId/sender-ids")
export class ClientSenderIdsController {
  constructor(
    private readonly senderIds: SenderIdService,
    private readonly mapper: SenderIdMapper,
  ) { }

  @Get()
  @Authorize(Permissions.SENDER_IDS_READ)
  @ApiOperation({
    summary: "Retrieve a paginated list of Sender IDs for a client.",
  })
  @ApiPaginatedResponse(SenderIdResponseDto)
  async findMany(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Query() dto: FindSenderIdsDto,
  ): Promise<PaginatedResponse<SenderIdResponseDto>> {
    const page = await this.senderIds.findMany({
      page: dto.page ?? 1,
      pageSize: dto.pageSize ?? 20,
      clientId,
      status: dto.status,
      sender: dto.sender,
      search: dto.search,
      isDefault: dto.isDefault,
    });

    return new PaginatedResponse(this.mapper.toResponses(page.items), page);
  }

  @Get(":id")
  @Authorize(Permissions.SENDER_IDS_READ)
  @ApiOperation({
    summary: "Retrieve a Sender ID for a client.",
  })
  @ApiParam({
    name: "id",
    description: "Sender ID identifier.",
  })
  @ApiSuccessResponse(SenderIdResponseDto)
  @ApiNotFoundResponse({ description: "Sender ID not found for client." })
  async findById(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<SenderIdResponseDto> {
    const entity = await this.senderIds.findById(id);

    if (entity.clientId !== clientId) {
      throw new NotFoundException();
    }

    return this.mapper.toResponse(entity);
  }

  @Post()
  @Authorize(Permissions.SENDER_IDS_CREATE)
  @ApiOperation({ summary: "Create a Sender ID for a client." })
  @ApiSuccessResponse(SenderIdResponseDto)
  async create(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Body() dto: CreateSenderIdDto,
  ): Promise<SenderIdResponseDto> {
    const toCreate = { ...dto, clientId } as CreateSenderIdDto;

    const created = await this.senderIds.create(toCreate);

    return this.mapper.toResponse(created);
  }

  @Put(":id")
  @Authorize(Permissions.SENDER_IDS_UPDATE)
  @ApiOperation({ summary: "Update a Sender ID for a client." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiSuccessResponse(SenderIdResponseDto)
  @ApiNotFoundResponse({ description: "Sender ID not found for client." })
  async update(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
    @Body() dto: UpdateSenderIdDto,
  ): Promise<SenderIdResponseDto> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    const updated = await this.senderIds.update(id, dto);

    return this.mapper.toResponse(updated);
  }

  @Delete(":id")
  @Authorize(Permissions.SENDER_IDS_DELETE)
  @ApiOperation({ summary: "Delete a Sender ID for a client." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiNotFoundResponse({ description: "Sender ID not found for client." })
  async delete(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<void> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    await this.senderIds.delete(id);
  }

  @Post(":id/approve")
  @Authorize(Permissions.SENDER_IDS_APPROVE)
  @ApiOperation({ summary: "Approve a Sender ID for a client." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiSuccessResponse(SenderIdResponseDto)
  async approve(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<SenderIdResponseDto> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    const result = await this.senderIds.approve(id);

    return this.mapper.toResponse(result);
  }

  @Post(":id/reject")
  @Authorize(Permissions.SENDER_IDS_REJECT)
  @ApiOperation({ summary: "Reject a Sender ID for a client." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiSuccessResponse(SenderIdResponseDto)
  async reject(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<SenderIdResponseDto> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    const result = await this.senderIds.reject(id);

    return this.mapper.toResponse(result);
  }

  @Post(":id/disable")
  @Authorize(Permissions.SENDER_IDS_DISABLE)
  @ApiOperation({ summary: "Disable a Sender ID for a client." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiSuccessResponse(SenderIdResponseDto)
  async disable(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<SenderIdResponseDto> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    const result = await this.senderIds.disable(id);

    return this.mapper.toResponse(result);
  }

  @Post(":id/enable")
  @Authorize(Permissions.SENDER_IDS_ENABLE)
  @ApiOperation({ summary: "Enable a Sender ID for a client." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiSuccessResponse(SenderIdResponseDto)
  async enable(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<SenderIdResponseDto> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    const result = await this.senderIds.enable(id);

    return this.mapper.toResponse(result);
  }

  @Post(":id/default")
  @Authorize(Permissions.SENDER_IDS_DEFAULT_UPDATE)
  @ApiOperation({ summary: "Set a Sender ID as the client's default." })
  @ApiParam({ name: "id", description: "Sender ID identifier." })
  @ApiSuccessResponse(SenderIdResponseDto)
  async setDefault(
    @Param("clientId", ParseUUIDPipe) clientId: string,
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<SenderIdResponseDto> {
    const existing = await this.senderIds.findById(id);

    if (existing.clientId !== clientId) {
      throw new NotFoundException();
    }

    const result = await this.senderIds.setDefault(id);

    return this.mapper.toResponse(result);
  }
}
