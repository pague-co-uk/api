...
import { RevokeSessionOptions } from "../dto/revoke-session-options.js";
import { SessionValidationResult } from "../dto/session-validation-result.js";
import { TouchSessionOptions } from "../dto/touch-session-options.js";
import { ValidateSessionOptions } from "../dto/validate-session-options.js";
      ...
      async (span) => {
        ...
        try {
          ...
          const session = await this.repository.create({
            ...
            trustedDeviceId: options.trustedDeviceId,
            authenticatedWithMfa:
              options.authenticatedWithMfa,
            lastActivityAt: now,
            expiresAt,
    ...
    return withSpan(
      ...
      async (span) => {
        ...
        const invalid = (
          ...
          this.sessionsValidationFailedCounter.add(
            1,
            {
              reason,
            },
        ...
        try {
          ...
          try {
            token = SessionToken.parse(options.token);
          } catch {
            return invalid(
              SessionValidationFailureReason.INVALID_TOKEN,
            );
          }
          ...
          if (session.revokedAt) {
            return invalid(
              SessionValidationFailureReason.REVOKED,
            );
          }
    ...
    return withSpan(
      ...
      async (span) => {
        ...
        try {
          ...
          const result =
            await this.repository.revokeAllForUser(
              userId,
              revokedAt,
            );
          ...
          this.sessionsRevokedAllCounter.add(
            1,
          );
