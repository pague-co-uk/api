...
export class RefreshTokenRepository extends DatabaseRepository {
  ...
  public withDatabase(db: Prisma.TransactionClient): this {
    return new RefreshTokenRepository(db) as this;
  }

  create(data: Prisma.RefreshTokenCreateInput) {
    return this.execute('INSERT', 'refresh_tokens', async () => ({
  ...
  findById(id: string) {
    return this.execute('SELECT', 'refresh_tokens', async () => {
  ...
  revoke(id: string, revokedAt: Date) {
    return this.execute('UPDATE', 'refresh_tokens', async () => ({
      result: await this.db.refreshToken.update({
        where: {
          id,
        },
        data: {
  ...
  revokeBySession(sessionId: string, revokedAt: Date) {
    return this.execute('UPDATE', 'refresh_tokens', async () => {
  ...
  revokeByUser(userId: string, revokedAt: Date) {
    return this.execute('UPDATE', 'refresh_tokens', async () => {
      const result = await this.db.refreshToken.updateMany({
        where: {
          session: {
  ...
  replace(id: string, replacedById: string, revokedAt: Date) {
    return this.execute('UPDATE', 'refresh_tokens', async () => ({
      result: await this.db.refreshToken.update({
        where: {
          id,
  ...
  deleteExpired(before: Date) {
    return this.execute('DELETE', 'refresh_tokens', async () => {
